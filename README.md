...
* **`packages/lexicon-importer`**：提供 `@sancho-ai-ime/lexicon-importer` 库和 `sancho-lexicon-importer` CLI，支持 Rime `custom_phrase.txt`、Rime `dict.yaml`、TSV 和 CSV 的预览、去重、权重标准化、导入输出和 rollback snapshot；导入结果属于用户数据，应写入 ignored runtime directory。
